...
console.log(__pvNodes() > 0);`;
  ...
  /*
   * INTEGRATION REGRESSIONS — every one of these crashed (exit 255) when the trie
   * landed, and NEITHER branch could have caught it alone: each routine was written
   * against the pre-trie array and reads the flat `data` pointer, which is NULL once
   * an array freezes past 32 elements. They are the reason a green branch is not a
   * green merge. Anything reading array slots in C must go through the accessor.
   */
  test("toSorted / toReversed work on a trie-backed array (>32)", async () => {
    const src = `${BUILD}
const a: number[] = build(200);
const s: number[] = a.toSorted((x: number, y: number): number => y - x);
const r: number[] = a.toReversed();
console.log(s.length, s[0], s[199], r[0], r[199]);
const t: string[] = build(50).map((n: number): string => \`k\${n}\`);
console.log(t.toSorted()[0], t.toReversed()[0]);`;
    const { ours, oracle } = await expectMatchesNode(src);
    expect(ours.stdout).toBe(oracle.stdout);
    expect(ours.exitCode).toBe(oracle.exitCode);
  });

  test("Uint8Array from a trie-backed number array (>32)", async () => {
    // nt_bytes.c used to keep its OWN copy of the NtArray layout, which went stale
    // the moment arrays grew trie storage. It now uses the exported accessors.
    const src = `${BUILD}
const u = new Uint8Array(build(100));
console.log(u.length, u[0], u[50], u[99]);`;
    const { ours, oracle } = await expectMatchesNode(src);
    expect(ours.stdout).toBe(oracle.stdout);
    expect(ours.exitCode).toBe(oracle.exitCode);
  });
