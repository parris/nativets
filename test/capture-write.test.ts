 ...
 * Refusal, not implementation. `NT1029` rejects an assignment / compound assignment /
 ...
 * copy there, so the env slot is the whole variable and node agrees with us. NT1029 is
...
/* ============================================================
 * WRITES are refused (NT1029), rather than compiled to the wrong answer.
 *
...
describe("a write to a captured binding is refused", () => {
  /** Refused with NT1029, and the message names the binding and the operator. */
  function expectRefused(source: string, op: string, name: string): void {
    const r = rejectionOf(source);
    expect(r?.code).toBe("NT1029");
...
`, "n = …", "n");
  ...
  test("a shadowing `let` in a NESTED BLOCK of the closure is refused, not silently confused", () => {
    ...
    // exit 0 on both. NT1029's wording calls this a capture, which is what the compiler
...
`);
  ...
  test("an INLINED map callback writing a binding the ENCLOSING frame owns still works", async () => {
    ...
    // the NT1029 hint points people at, so it has to keep working — and it is the reason
