...
export function check(program: Program): CheckedProgram {
  ...
  c.bodyChain = [program.body]; // the outermost enclosing body, for NT1029
...
class Checker {
   ...
   * `checkCapturedWrites` (NT1029) reads it: deciding whether a closure's write to a
  ...
  private typeArrowBody(arrow: Extract<Expr, { kind: "ArrowFunction" }>, paramTypes: Ty[], scope: Scope): Ty {
    ...
    // enclosing chain (NT1029) — the callback's own statements are one of the places an
